...
public class ScaleCircleNavigator extends View
  ...
  private int measureWidth(int widthMeasureSpec) {
    ...
    switch (mode) {
      ...
      case MeasureSpec.UNSPECIFIED:
        result = (mCircleCount - 1) * mMinRadius * 2
            + mMaxRadius * 2
            + (mCircleCount - 1) * mCircleSpacing
            + getPaddingLeft()
            + getPaddingRight();
        break;
      default:
  ...
  public void onEnter(int index, int totalCount, float enterPercent, boolean leftToRight) {
    if (mFollowTouch) {
      float radius = mMinRadius + (mMaxRadius - mMinRadius) * mStartInterpolator.getInterpolation(
          enterPercent);
  ...
  public void onLeave(int index, int totalCount, float leavePercent, boolean leftToRight) {
    if (mFollowTouch) {
      float radius = mMaxRadius + (mMinRadius - mMaxRadius) * mStartInterpolator.getInterpolation(
          leavePercent);
      mCircleRadiusArray.put(index, radius);
      invalidate();
    }
