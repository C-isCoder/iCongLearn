  ...
  void OnClick(View v) {
    switch (v.getId()) {
      case R.id.button:
        ...
        break;
      case R.id.button1:
        BCDialogUtil.getDialogItem(getAty(), "标题", ITEMS, (dialog, which) -> {
      ...
      case R.id.button3:
        ...
        break;
      case R.id.button4:
        BCDialogUtil.getInputDialog(getAty(), new EditText(getAty()), "标题", (dialog, which) -> {
          showMessage("确认");
        }, (dialog, which) -> {
          showMessage("取消");
        });
        break;
      case R.id.button5:
        BCDialogUtil.showDialog(getAty(), "标题", "内容", (dialog, which) -> {
          showMessage("确定");
        }, (dialog, which) -> {
          showMessage("取消");
        });
